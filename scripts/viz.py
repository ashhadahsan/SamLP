...
def get_random_data_point(root_dir):
    ...
    track_dirs = [d for d in glob.glob(os.path.join(root_dir_train, "track*")) if os.path.isdir(d)]
...
def extract_metadata(data:str):
    ...
    response={"char_positions":char_positions,"vehicle_position":vehicle_position,
    "corners":corners,"camera":camera,"vehicle_type":type_v,"vehicle_make":type_make,
    "model_type":type_model,"model_year":type_year,"model_plate":type_plate
    }
    return response
